...
import com.neulketing.openthumb.config.collections.EnvVarsCollection
import com.neulketing.openthumb.service.DynamicIslandSupport
import com.neulketing.openthumb.config.collections.GroupsCollection
import com.neulketing.openthumb.config.collections.ModelsCollection
import com.neulketing.openthumb.config.collections.ProvidersCollection
import com.neulketing.openthumb.config.fields.ClosureField
import com.neulketing.openthumb.config.fields.PrefsBoolField
import com.neulketing.openthumb.config.fields.PrefsDoubleField
import com.neulketing.openthumb.config.fields.PrefsEnumField
import com.neulketing.openthumb.config.fields.PrefsIntCodedEnumField
import com.neulketing.openthumb.config.fields.PrefsIntField
import com.neulketing.openthumb.config.fields.PrefsLongField
import com.neulketing.openthumb.config.fields.PrefsStringField
...
internal object ConfigBuiltins {
    ...
    private fun registerAppearance(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = prefs,
                key = com.neulketing.openthumb.ui.settings.KEY_SHOW_CHAT_TITLE,
                defaultValue = true,
            )
        )
        r.register(
    ...
    private fun registerChat(r: ConfigRegistry, context: Context) {
        ...
        val appearancePrefs = context.getSharedPreferences(
            com.neulketing.openthumb.ui.settings.PREF_APPEARANCE,
        ...
        r.register(
            PrefsBoolField(
                ...
                key = com.neulketing.openthumb.ui.settings.KEY_TOOL_PREVIEW,
                defaultValue = true,
            )
        )
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = com.neulketing.openthumb.ui.settings.KEY_AUTO_FOCUS_AFTER_REPLY,
                defaultValue = true,
            )
        )
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.neulketing.openthumb.ui.settings.KEY_FONT_CHAT_INPUT,
        ))
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.neulketing.openthumb.ui.settings.KEY_FONT_MESSAGE,
        ))
    }
    ...
    private fun registerLogs(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            ClosureField(
                ...
                revertable = true,
                reader = {
                    ConfigValue.Bool(com.neulketing.openthumb.logging.AppLogger.isEnabled(context))
                },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.neulketing.openthumb.logging.AppLogger.setEnabled(context, b)
                },
            )
        )
    }
    ...
    private fun registerProviderCollections(
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val providersById = HashMap<String, com.neulketing.openthumb.data.model.ProviderInstance>(cfg.instances.size)
        ...
        r.register(
            ClosureField(
                ...
                risk = ConfigRisk.SENSITIVE,
                revertable = true,
                reader = { ConfigValue.Bool(com.neulketing.openthumb.data.EnvVarPrivacyStore.isEnabled) },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.neulketing.openthumb.data.EnvVarPrivacyStore.setEnabled(b)
                },
            )
        )
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val entriesById =
                        HashMap<String, com.neulketing.openthumb.data.model.ModelEntry>(cfg.modelEntries.size)
                    for (e in cfg.modelEntries) entriesById[e.id] = e
                    val providersById =
                        HashMap<String, com.neulketing.openthumb.data.model.ProviderInstance>(cfg.instances.size)
                    for (inst in cfg.instances) providersById[inst.id] = inst
                    ConfigValue.Arr(
    ...
    private fun registerSoul(r: ConfigRegistry, context: Context) {
        ...
        // English ≤ 500 words); see [com.neulketing.openthumb.agent.SoulStore.isOverLimit].
        ...
        fun loadCurrent(): com.neulketing.openthumb.agent.SoulFile =
            com.neulketing.openthumb.agent.SoulStore.load(context)
                ?: com.neulketing.openthumb.agent.SoulMDParser.parse(
                    com.neulketing.openthumb.agent.SoulStore.DEFAULT_CONTENT
                )

        fun saveCurrent(file: com.neulketing.openthumb.agent.SoulFile) {
            com.neulketing.openthumb.agent.SoulStore.save(context, file)
        }
            ...
            ClosureField(
                ...
                writer = { v ->
                    ...
                    val name = raw.trim().ifBlank {
                        com.neulketing.openthumb.agent.SoulMetadata.DEFAULT.name
                    }
                    val cur = loadCurrent()
            ...
            ClosureField(
                ...
                reader = {
                    ...
                    ConfigValue.Str(cur.metadata.lang.ifBlank {
                        com.neulketing.openthumb.agent.SoulMetadata.DEFAULT.lang
                    })
                },
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
        ...
        r.register(
            ClosureField(
                ...
                description = "Personality / voice instructions injected as a block in the system prompt. Length cap is language-aware: Chinese ≤ ${com.neulketing.openthumb.agent.SoulStore.CHINESE_CHAR_LIMIT} chars OR English ≤ ${com.neulketing.openthumb.agent.SoulStore.ENGLISH_WORD_LIMIT} words (rule picked by the majority language). The writer also rejects prompt-injection patterns (\"ignore previous instructions\" etc.) — keep this to genuine character / tone guidance.",
                ...
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
                        ?: throw ConfigError.TypeMismatch("string")
                    if (com.neulketing.openthumb.agent.SystemPromptBuilder
                    ...
                    val check = com.neulketing.openthumb.agent.SoulStore.isOverLimit(raw)
                    when (check) {
                        is com.neulketing.openthumb.agent.SoulBodyLimitCheck.Ok -> Unit
                        is com.neulketing.openthumb.agent.SoulBodyLimitCheck.OverLimitChinese ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.chars} Chinese chars exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} chars, or rewrite mostly in English to use the ≤ ${com.neulketing.openthumb.agent.SoulStore.ENGLISH_WORD_LIMIT}-word cap."
                            )
                        is com.neulketing.openthumb.agent.SoulBodyLimitCheck.OverLimitEnglish ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.words} English words exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} words, or rewrite mostly in Chinese to use the ≤ ${com.neulketing.openthumb.agent.SoulStore.CHINESE_CHAR_LIMIT}-char cap."
                            )
                    }
                    val cur = loadCurrent()
