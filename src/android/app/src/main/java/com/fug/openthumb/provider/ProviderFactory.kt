package com.fug.openthumb.provider
...
import com.fug.openthumb.provider.anthropic.AnthropicProvider
import com.fug.openthumb.provider.gemini.GeminiProvider
import com.fug.openthumb.provider.openai.OpenAIProvider
        ...
        return when (instance.providerType) {
            ...
            ProviderType.openAI -> {
                ...
                val manualBearer = if (context != null &&
                    ...
                    com.fug.openthumb.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    ...
                    OpenAIProvider(
                        oauthTokenProvider = { oauthManager.validAccessToken() ?: throw com.fug.openthumb.data.model.LLMError.InvalidApiKey() },
                        model = model,
            ...
            ProviderType.xAI -> {
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.fug.openthumb.auth.XAIOAuthManager(context, instance.id)
                    OpenAIProvider.oauthOpenAICompat(
                        oauthTokenProvider = {
                            oauthManager.validAccessToken()
                                ?: throw com.fug.openthumb.data.model.LLMError.InvalidApiKey()
                        },
                        model = model,
                        basePath = base,
                    )
            ...
            ProviderType.kimiCode -> {
                ...
                val base = basePath ?: "${com.fug.openthumb.auth.KimiDeviceFlow.CODING_API_BASE}/v1"
                val manualBearer = if (context != null &&
                    instance.credentialType == ProviderCredential.oauth) {
                    com.fug.openthumb.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.fug.openthumb.auth.KimiOAuthManager(context, instance.id)
