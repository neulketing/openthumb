package com.fug.openthumb.debug
...
class DebugRPCHandler(private val context: Context) {
    ...
    private suspend fun handleShellExecute(params: JSONObject): JSONObject {
        ...
        val result = try {
            ExecutionCoordinator.execute(
                ...
                lineCallback = { rawLine ->
                    val (_, urls) = com.fug.openthumb.terminal.MinisUrlMarker.extract(rawLine)
        ...
        for (raw in capturedUrls) {
            com.fug.openthumb.terminal.MinisOpenUrlBroker.offer(raw)
        }
        return JSONObject()
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = com.fug.openthumb.data.UpdateChecker.check()) {
            is com.fug.openthumb.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            com.fug.openthumb.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            com.fug.openthumb.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is com.fug.openthumb.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            com.fug.openthumb.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            com.fug.openthumb.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is com.fug.openthumb.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = com.fug.openthumb.data.UpdateChecker.download(context, url)) {
            is com.fug.openthumb.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is com.fug.openthumb.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = com.fug.openthumb.data.UpdateChecker.canInstall(context)
        return try {
            com.fug.openthumb.data.UpdateChecker.installApk(context, file)
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = PRootKernel.resolveHostPath(path) ?: run {
            val rootfsDir = com.fug.openthumb.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in com.fug.openthumb.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", com.fug.openthumb.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [com.fug.openthumb.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = com.fug.openthumb.sandbox.offload.ShizukuOffloadHandler(context)
        ...
        val request = com.fug.openthumb.sandbox.NativeOffloadRequest(
            pid = -1,
    ...
    /**
     * Direct invocation of [com.fug.openthumb.sandbox.offload.ModelUseOffloadHandler]
    ...
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = com.fug.openthumb.sandbox.PRootKernel.resolveHostPath(linuxPath)
        ...
        val app = context.applicationContext as com.fug.openthumb.MinisApp
        val handler = com.fug.openthumb.sandbox.offload.ModelUseOffloadHandler(context, app.providerRepository)
        val request = com.fug.openthumb.sandbox.NativeOffloadRequest(
            pid = -1,
     ...
     * [com.fug.openthumb.sandbox.offload.SessionsOffloadHandler] for e2e
    ...
    private fun handleSessionsExec(params: JSONObject): JSONObject {
        ...
        val app = context.applicationContext as com.fug.openthumb.MinisApp
        val handler = com.fug.openthumb.sandbox.offload.SessionsOffloadHandler(app.chatRepository)
        val request = com.fug.openthumb.sandbox.NativeOffloadRequest(
            pid = -1,
        ...
        return when (sub) {
            "set" -> {
                ...
                kotlinx.coroutines.runBlocking {
                    com.fug.openthumb.config.ConfigBridge.performWriteBatch(
                        items = items,
            ...
            "get" -> {
                ...
                com.fug.openthumb.config.ConfigBridge.readField(
                    path = path,
                    filter = null,
                    page = 0,
                    pageSize = 0,
            ...
            "audit-list" -> {
                ...
                com.fug.openthumb.config.ConfigBridge.auditList(limit, null)
            }
