package com.fug.openthumb.sandbox.offload
...
import com.fug.openthumb.provider.safeOptString
import com.fug.openthumb.sandbox.NativeOffloadHandler
import com.fug.openthumb.sandbox.NativeOffloadRequest
import com.fug.openthumb.sandbox.NativeOffloadResult
...
class ModelUseOffloadHandler(
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        ...
        if (hasAudioInput && provider !is com.fug.openthumb.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        if (ptSpec.active) {
            val openAI = provider as? com.fug.openthumb.provider.openai.OpenAIProvider
                ?: return NativeOffloadResult(
                    2,
        ...
        (provider as? com.fug.openthumb.provider.openai.OpenAIProvider)?.let { openAI ->
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: com.fug.openthumb.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> com.fug.openthumb.data.model.ImageEndpointMode.auto
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: com.fug.openthumb.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        ...
        provider: com.fug.openthumb.provider.LLMProvider,
        inputJson: String,
        ...
        if (instance.credentialType == com.fug.openthumb.data.model.ProviderCredential.oauth) return null
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> com.fug.openthumb.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == com.fug.openthumb.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == com.fug.openthumb.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == com.fug.openthumb.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
        ...
        val response = try {
            runBlocking { openAI.generateImage(prompt, cfg.n, cfg.size, cfg.quality) }.also {
                providerRepository.setImageEndpointResolved(
                    instance.id, com.fug.openthumb.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            val routeMissing = (e is com.fug.openthumb.data.model.LLMError.ProviderError ||
                e is com.fug.openthumb.data.model.LLMError.InvalidApiKey) && looksLikeEndpointMissing(msg)
            if (routeMissing) {
                Log.i(TAG, "[ModelUseRoute] auto probe → route missing (${msg.take(120)}) — caching chat_completions, falling back")
                providerRepository.setImageEndpointResolved(
                    instance.id, com.fug.openthumb.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: com.fug.openthumb.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        com.fug.openthumb.offload.ModelUseManager.entryDict(
            entry,
