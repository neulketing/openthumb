package com.fug.openthumb.config

import android.content.Context
import com.fug.openthumb.config.collections.EnvVarsCollection
import com.fug.openthumb.service.DynamicIslandSupport
import com.fug.openthumb.config.collections.GroupsCollection
import com.fug.openthumb.config.collections.ModelsCollection
import com.fug.openthumb.config.collections.ProvidersCollection
import com.fug.openthumb.config.fields.ClosureField
import com.fug.openthumb.config.fields.PrefsBoolField
import com.fug.openthumb.config.fields.PrefsDoubleField
import com.fug.openthumb.config.fields.PrefsEnumField
import com.fug.openthumb.config.fields.PrefsIntCodedEnumField
import com.fug.openthumb.config.fields.PrefsIntField
import com.fug.openthumb.config.fields.PrefsLongField
import com.fug.openthumb.config.fields.PrefsStringField
import com.fug.openthumb.config.fields.ReadOnlyField
...
import com.fug.openthumb.ui.chat.ChatViewModelStore
...
internal object ConfigBuiltins {
    ...
    private fun registerAppearance(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = prefs,
                key = com.fug.openthumb.ui.settings.KEY_SHOW_CHAT_TITLE,
                defaultValue = true,
            )
        )
        r.register(
    ...
    private fun registerChat(r: ConfigRegistry, context: Context) {
        ...
        val appearancePrefs = context.getSharedPreferences(
            com.fug.openthumb.ui.settings.PREF_APPEARANCE,
        ...
        r.register(
            PrefsBoolField(
                ...
                key = com.fug.openthumb.ui.settings.KEY_TOOL_PREVIEW,
                defaultValue = true,
            )
        )
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = com.fug.openthumb.ui.settings.KEY_AUTO_FOCUS_AFTER_REPLY,
                defaultValue = true,
            )
        )
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.fug.openthumb.ui.settings.KEY_FONT_CHAT_INPUT,
        ))
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.fug.openthumb.ui.settings.KEY_FONT_MESSAGE,
        ))
    }
    ...
    private fun registerLogs(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            ClosureField(
                ...
                revertable = true,
                reader = {
                    ConfigValue.Bool(com.fug.openthumb.logging.AppLogger.isEnabled(context))
                },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.fug.openthumb.logging.AppLogger.setEnabled(context, b)
                },
            )
        )
    }
    ...
    private fun registerProviderCollections(
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val providersById = HashMap<String, com.fug.openthumb.data.model.ProviderInstance>(cfg.instances.size)
        ...
        r.register(
            ClosureField(
                ...
                risk = ConfigRisk.SENSITIVE,
                revertable = true,
                reader = { ConfigValue.Bool(com.fug.openthumb.data.EnvVarPrivacyStore.isEnabled) },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.fug.openthumb.data.EnvVarPrivacyStore.setEnabled(b)
                },
            )
        )
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val entriesById =
                        HashMap<String, com.fug.openthumb.data.model.ModelEntry>(cfg.modelEntries.size)
                    for (e in cfg.modelEntries) entriesById[e.id] = e
                    val providersById =
                        HashMap<String, com.fug.openthumb.data.model.ProviderInstance>(cfg.instances.size)
                    for (inst in cfg.instances) providersById[inst.id] = inst
                    ConfigValue.Arr(
    ...
    private fun registerSoul(r: ConfigRegistry, context: Context) {
        ...
        // English ≤ 500 words); see [com.fug.openthumb.agent.SoulStore.isOverLimit].
        ...
        fun loadCurrent(): com.fug.openthumb.agent.SoulFile =
            com.fug.openthumb.agent.SoulStore.load(context)
                ?: com.fug.openthumb.agent.SoulMDParser.parse(
                    com.fug.openthumb.agent.SoulStore.DEFAULT_CONTENT
                )

        fun saveCurrent(file: com.fug.openthumb.agent.SoulFile) {
            com.fug.openthumb.agent.SoulStore.save(context, file)
        }
            ...
            ClosureField(
                ...
                writer = { v ->
                    ...
                    val name = raw.trim().ifBlank {
                        com.fug.openthumb.agent.SoulMetadata.DEFAULT.name
                    }
                    val cur = loadCurrent()
            ...
            ClosureField(
                ...
                reader = {
                    ...
                    ConfigValue.Str(cur.metadata.lang.ifBlank {
                        com.fug.openthumb.agent.SoulMetadata.DEFAULT.lang
                    })
                },
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
        ...
        r.register(
            ClosureField(
                ...
                description = "Personality / voice instructions injected as a block in the system prompt. Length cap is language-aware: Chinese ≤ ${com.fug.openthumb.agent.SoulStore.CHINESE_CHAR_LIMIT} chars OR English ≤ ${com.fug.openthumb.agent.SoulStore.ENGLISH_WORD_LIMIT} words (rule picked by the majority language). The writer also rejects prompt-injection patterns (\"ignore previous instructions\" etc.) — keep this to genuine character / tone guidance.",
                ...
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
                        ?: throw ConfigError.TypeMismatch("string")
                    if (com.fug.openthumb.agent.SystemPromptBuilder
                    ...
                    val check = com.fug.openthumb.agent.SoulStore.isOverLimit(raw)
                    when (check) {
                        is com.fug.openthumb.agent.SoulBodyLimitCheck.Ok -> Unit
                        is com.fug.openthumb.agent.SoulBodyLimitCheck.OverLimitChinese ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.chars} Chinese chars exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} chars, or rewrite mostly in English to use the ≤ ${com.fug.openthumb.agent.SoulStore.ENGLISH_WORD_LIMIT}-word cap."
                            )
                        is com.fug.openthumb.agent.SoulBodyLimitCheck.OverLimitEnglish ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.words} English words exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} words, or rewrite mostly in Chinese to use the ≤ ${com.fug.openthumb.agent.SoulStore.CHINESE_CHAR_LIMIT}-char cap."
                            )
                    }
                    val cur = loadCurrent()
